...
import static de.blankedv.andropanel.AndroPanelApplication.TURNOUT_LENGTH;
import static de.blankedv.andropanel.AndroPanelApplication.TURNOUT_LENGTH_LONG;
...
public class LinearMath {
	...
	/**
	  ...
	  public static PanelElement trackIntersect(
			  PanelElement e, PanelElement f ) {
		  
		  // only look for crossing track elements
		  if (!e.getType().equals("track") || (!f.getType().equals("track")) ) return null;  
	    ...
	    int xt,yt, xc, yc;  // for turnout x-thrown, y-thrown, x-closed, y-closed
	    if (   (xi>=Math.min(x1, x2)) && (xi<=Math.max(x1, x2))  // within x-limits of first line
	    	&& (xi>=Math.min(x3, x4)) && (xi<=Math.max(x3, x4))  // within x-limits of second line
	    	&& (yi>=Math.min(y1, y2)) && (yi<=Math.max(y1, y2))  // within y-limits of first line
	    	&& (yi>=Math.min(y3, y4)) && (yi<=Math.max(y3, y4)) ) {   // within y-limits of second line
	    	
	    	// check if point is not endpoint of both tracks => no turnout
	    	if ( ((xi == e.x) && (yi == e.y))   || // startpoint of e ( e=thrown, f=closed)
	    		 ((xi == e.x2) && (yi == e.y2)) ) {  // endpoint of e  => delta-x immer negativ
	    		// start/endpoint of e
	    		if ( ((xi == f.x) && (yi == f.y))   ||  
	    			 ((xi == f.x2) && (yi == f.y2)) ) {
	    	...
	    	// 1. check, to which lines' endpoint (xi,yi) belongs
	    	...
	    	} else if ((xi == e.x2) && (yi == e.y2)) {  // endpoint of e  => delta-x immer negativ
	    		if ((e.y2-e.y)!=0) {  // y steigung immer = +/- 1
	    			if(e.y2>e.y) {  
	    				xt = xi-TURNOUT_LENGTH; 
		    			yt = yi-TURNOUT_LENGTH; 
		    ...
		    return new PanelElement("turnout", px, new Point(xc,yc), new Point(xt,yt));
	    } else {
